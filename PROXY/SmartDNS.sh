...
get_unlock_ip() {
    echo ""
    echo "========================================"
    print_info "配置解锁机IP地址"
    echo "========================================"
    echo ""
    print_warning "⚠️  重要提醒："
    echo "  1. 解锁机器必须已安装并运行 SNI Proxy"
    echo "  2. SNI Proxy 需要监听 80 和 443 端口"
    echo "  3. 防火墙需要开放这些端口"
    echo ""
    print_info "将配置以下域名指向解锁机："
    echo "  • Netflix, Disney+, HBO Max"
    echo "  • Hulu, Amazon Prime Video"
    echo "  • 日本/台湾/韩国等地区流媒体"
    echo "  • 共计 450+ 个域名"
    echo ""
...
show_menu() {
    ...
    echo ""
    echo -e "${BLUE}域名数据源：${NC}"
    echo "  https://github.com/1-stream/1stream-public-utils"
    echo "  (stream.text.list - 包含450+流媒体域名)"
    echo ""
    echo -e "${RED}⚠️  解锁机器要求：${NC}"
    echo -e "  ${RED}必须安装并运行 SNI Proxy 服务${NC}"
    echo "  否则流媒体解锁将无法工作！"
    echo ""
    echo -e "${GREEN}工作原理：${NC}"
    echo "  所有流媒体域名 → 解锁机IP → SNI Proxy → 目标服务"
    echo ""
    echo "========================================"
    echo ""
    echo "请选择操作："
...
EOF
    ...
    echo ""
    echo "========================================"
    echo -e "${GREEN}✅ SmartDNS 配置完成${NC}"
    echo "========================================"
    echo ""
    print_warning "⚠️  最后确认："
    echo ""
    echo "1. 确保解锁机器已安装 SNI Proxy："
    echo "   apt-get install sniproxy"
    echo ""
    echo "2. 确保 SNI Proxy 正在运行："
