...
install_app() {
    ...
    read -p "请输入密码（留空将自动生成16位随机密码）: " MIMA
    MIMA=${MIMA:-$(tr -dc A-Za-z0-9 </dev/urandom | head -c16)}
...
V2rayN
anytls://${MIMA}@${SERVER_IP}:${PORT}/?insecure=1#$HOSTNAME
Surge
$HOSTNAME = anytls, ${SERVER_IP}, ${PORT}, password=${MIMA}, tfo=true, skip-cert-verify=true, reuse=false
EOF
    read -p "按回车返回菜单..."
}
